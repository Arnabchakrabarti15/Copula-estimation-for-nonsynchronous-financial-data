...
compare = cbind(uncorrected_rho, rep(rho_ken, length(uncorrected_rho)))
#unc.est = mean(uncorrected_est[-1])
#unc.rho = mean(uncorrected_rho[-1])
#unc.rho.sd = sd(uncorrected_rho[-1])
#compare = rbind(compare, c( unc.est, rho_val))
compare2 = rbind(compare2, compare)
}
#xxx = compare[-1,1]
#yyy = compare[-1,2]
#par(mfrow=c(1,2))
...
yyy2 = compare2[-1,2]
op <- par(cex = 1.2)
plot(xxx2, yyy2, pch=20, xlab= "Estimated tau (uncorrected)", 
	ylab = "True value of tau", 
	main = "")
#abline(lm(yyy2~xxx2), col= "red")
fit2 <- lm(yyy2~poly(xxx2,2,raw=TRUE))
xx <- sort(runif(150,0, 1))
lines(xx, predict(fit2, data.frame(xxx2=xx)), col="red", lwd=2) 
summary(fit2)
coef(fit2)
pred.interval = predict(fit2, data.frame(xxx2=xx), interval="predict")
lines(xx, predict(fit2, data.frame(xxx2=xx), interval="predict")[,2], col="blue", lty=2, lwd=2)
lines(xx, predict(fit2, data.frame(xxx2=xx), interval="predict")[,3], col="blue", lty=2, lwd=2)

save_data = data.frame(xxx2,yyy2)
write.csv(save_data, "clayton_PI_fig_9_right.csv")
